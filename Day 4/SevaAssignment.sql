1.select C.company_name As Customer,O.order_id,P.product_id,D.quantity,O.order_date
from customers C inner join orders O on C.customer_id=o.customer_id 
inner join order_details D on O.order_id = D.order_id inner join products P on D.product_id=P.product_id 
order by order_id

2.select O.order_id,C.company_name as customer, E.first_name || ' ' || E.last_name as employeename, S.company_name, P.product_name, D.quantity, O.order_date
from orders O
...
3.select O.order_id,P.product_id,O.quantity,P.product_name
...
6.select E.employee_id, E.first_name || ' ' ||E.last_name as employeename,
